...
test.describe('Training Flow', () => {
		...
		if (testInfo.status !== testInfo.expectedStatus) {
			...
			try {
				...
				await page.screenshot({
					path: path.join(
						artifactsDir,
						`${testInfo.title}-${timestamp}-failure.png`,
					),
				})
				...
				await fs.writeFile(
					path.join(
						artifactsDir,
						`${testInfo.title}-${timestamp}-failure.html`,
					),
					html,
				)
			} catch (error: unknown) {
	...
	test.afterAll(async () => {
		...
		try {
			...
			await fs.rm(path.join(process.cwd(), 'test-artifacts', uniqueId), {
				recursive: true,
				force: true,
			})
	...
	test('should handle multiple file uploads', async ({ page }) => {
		...
		const secondTestAudioPath = path.join(
			fixturesDir,
			`test-audio-${uniqueId}-2.wav`,
		)
		...
		try {
			...
			try {
				...
				await expect(page.getByTestId('file-count')).toHaveText(
					'2 files selected',
				)
			...
			try {
				...
				await expect(page.getByTestId('past-samples')).toBeVisible({
					timeout: 10000,
				})
	...
	test('should allow deleting training files', async ({ page }) => {
		...
		await expect(page.getByTestId('past-samples').locator('audio')).toHaveCount(
			0,
		)
	...
	test('should generate audio after training', async ({ page }) => {
		...
		await expect(
			page.getByTestId('generated-audio').locator('audio'),
		).toBeVisible()

		// Verify audio source is set
		const audioSrc = await page
			.getByTestId('generated-audio')
			.locator('audio')
			.getAttribute('src')
	...
	test('should display training interface for admin users', async ({
		page,
	}) => {
	...
	test('should handle training process', async ({ page }) => {
		...
		try {
			await Promise.race([
				...
				page
					.getByTestId('file-error')
					.waitFor()
					.then(() => {
						throw new Error('File error detected')
					}),
			])
	...
	test.afterEach(async ({ page }, testInfo) => {
		...
		if (testInfo.status !== testInfo.expectedStatus) {
			await page.screenshot({
				path: `test-artifacts/${testInfo.title}-failure.png`,
			})
