...
def train():
    ...
    for stage_name, duration_s, epochs in CURRICULUM:
        ...
        for epoch in range(start_epoch, start_epoch + epochs):
            for real in tqdm(loader, desc=f"{stage_name}"):
                ...
                for _ in range(N_CRITIC):
                    ...
                    gp = compute_gradient_penalty(discriminator, real, fake, DEVICE, LAMBDA_GP)
                    (loss_D + gp).backward(); optim_D.step(); optim_D.zero_grad()
                # G step
                z = torch.randn(BATCH_SIZE, LATENT_DIM, 1, device=DEVICE)
                loss_G = -discriminator(generator(z)).mean()
                loss_G.backward(); optim_G.step(); optim_G.zero_grad()
        # save and generate
        ...
        torchaudio.save(f"{GENERATED_DIR}/{stage_name}_epoch{epoch+1}.wav", sample[:, :, :seq_len].cpu().squeeze(0), SAMPLE_RATE)
        start_epoch += epochs